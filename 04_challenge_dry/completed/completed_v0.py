...
#? generic single process_order ? need an if anyway so does not really improves, strategy pattern ?
...
def process_online_order(order: Order) -> None:
    ...
    #! ---- this is the only extra step for online order ---
...
def main() -> None:
    # ? items belongs to Order -> order.items = items
    items = [
